...
A timely stop atomically persists `cancel_requested`, invalidates result applicability and leaves the obligation open. Every subsequent native child dispatch is refused. The executor also rechecks the captured child handle after approval, start-order and owner waits, immediately before tool I/O or publishing a reused result. A tool waiting at those boundaries is not an already-issued effect. When no tool/cleanup/handoff is in progress the existing hard interrupt is delivered immediately. An already-dispatched tool is allowed to exit its ordinary cancellation boundary before signalling; the sealed fence survives inference expiry. No main-model approval is requested.
...
The host retires provider state on runtime revoke and distinct-work admission using the captured old work/revision and original registration recipients. `finish_turn` retires an idle scope, but never one with unsettled direct children. The recurring controller joins last-child durable settlement with parent completion; children finishing while the parent is still executing do not close its scope. Unknown settlement remains live. Same-work steering/new turns retain live child authority. Retirement is provider housekeeping, not obligation completion or effect reconciliation.

An admitted observation holds a delivery ticket until its scheduling callback returns. Terminal callbacks run outside runtime/control/registration/SQLite locks, after those tickets drain, and cannot target a replacement registration. Native monotonic event sequences fence delayed closures against newer observations, including post-turn completion owners at an unchanged revision. The standalone adapter reserves bounded terminal-delivery capacity even when its ordinary inference queue is full. Closed scopes release revision, sequence, cache, scheduler and arbiter state instead of accumulating against the 512-live-scope limit. Use the paired provider lifecycle implementation; an older provider does not supply the sequence/reserved-capacity guarantees.
...
`tests/agent/test_supervision_scope_lifecycle.py` additionally exercises both parent/child completion orders, last-sibling settlement, same-work continuation, revoke, distinct-work retirement, delayed old delivery versus a replacement turn, and 520 completed native scopes against the real standalone adapter. The direct owner, legacy bridge, configured policy and native children suites retain mechanical authority, race, persistence failure and settlement checks under the one-decision contract.
